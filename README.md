...
- 시도한 방법 : kubectl 명령어로 수동 생성하는 대신 수동 개입을 최소화하기 위해 ArgoCD의 동기화 자동화 옵션을 조사
- 최종 해결 : Dev 환경에서 해당 에러를 겪은 후 ArgoCD 매니페스트에 syncOptions: - CreateNamespace=true 옵션을 적용하고 Prod 환경에도 선제적으로 반영
- 배운 점 : 개발 환경에서 겪은 인프라 시행착오를 바탕으로 네임스페이스 생성 같은 기초 세팅까지 터미널 수동 작업 없이 코드로 자동화하는 완전한 GitOps 파이프라인의 가치를 실현
